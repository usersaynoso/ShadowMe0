...
-- This script will:
-- 1. Create all enums
-- 2. Create all tables in the correct order to handle dependencies
...
-- Create emotions table (no dependencies)
...
-- Create profiles table (depends on users)
...
-- Create friends table (depends on users)
...
-- Create friend_groups table (depends on users)
...
-- Create friend_group_members table (depends on friend_groups and users)
...
-- Create groups table (depends on users)
CREATE TABLE groups (
...
-- Create group_members table (depends on groups and users)
...
-- Create post_audience table (depends on posts and friend_groups)
...
-- Create post_comments table (depends on posts and users)
...
-- Create post_reactions table (depends on posts and users)
...
-- Create shadow_sessions table (depends on posts)
...
-- Create shadow_session_participants table (depends on shadow_sessions and users)
...
-- Create chat_rooms table (no dependency on foreign keys)
...
-- Create chat_room_members table (depends on chat_rooms and users)
...
-- Create feed_events table (depends on users)
