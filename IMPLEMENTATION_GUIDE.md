...
> **Note**: The Shadow Session components have been implemented in the project. The `CreateShadowSession`, `CreateShadowSessionDialog`, and `TimePicker` components are now available in the codebase. The examples below show the implementation details.
...
### Adding Avatar Upload to Profile Edit

The avatar upload functionality is integrated into the ProfileEditDialog component:

```tsx
// Inside ProfileEditDialog component
const { user } = useAuth();
const [avatarFile, setAvatarFile] = useState<File | null>(null);
const [avatarPreview, setAvatarPreview] = useState<string | null>(null);
...
### Server-Side Avatar Upload Handler

The server handles avatar uploads using multer and Supabase:
...
This implementation provides a seamless avatar upload experience with client-side preview, validation, and appropriate error handling, while ensuring the backend storage is robust with fallback mechanisms.
